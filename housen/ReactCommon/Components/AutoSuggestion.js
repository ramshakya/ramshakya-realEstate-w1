...
const AutoSuggestion = (props) => {
  ...
  useEffect(() => {
    ...
    if (
      JSON.stringify(props.allList) !== JSON.stringify(allList) &&
      !props.autoCompleteCb
    ) {
  ...
  const handleOuterClick = (e) => {
    if (
      inputDivRef !== null &&
      inputDivRef.current !== null &&
      !inputDivRef.current.contains(e.target)
    ) {
      setShowList(false);
    }
  };
  ...
  const renderList = () => {
    ...
    const renderList = allList.map((data, index) => {
      ...
      return (
        ...
        >
          {data.text}
          {/* <span className="short-text">{data.shortText}</span> */}
        </li>
      );
    });
  ...
  return (
    <div
      id={`${extraProps.id ? extraProps.id : ""}`}
      className={`sss ${extraProps.parentcls ? extraProps.parentcls : ""}  ${
        Style.autoSuggestionCls
      }`}
      ref={inputDivRef}
    >
      {
        props.showIcon &&
      <i className="fa fa-sort-down community-icon" onClick={handleOnFocus}></i>
      }
      <input
        {...inputProps}
