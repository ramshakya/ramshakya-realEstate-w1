...
class ListingsV2 extends React.Component {
  ...
  handleTypeHead(obj = null, name = null) {
    ...
    const { propertySearchFilter } = this.state;
    if(this.state.isPopularSearched){
      propertySearchFilter["curr_bounds"] = "";
      const filters = Object.keys(propertySearchFilter);
      if (filters && filters.length) {
        filters.map((item, k) => {
          if (item === "Dom") {
            propertySearchFilter[item] = "90";
          } else if (item === "soldStatus") {
            propertySearchFilter[item] = "A";
          } 
          else if (item === "text_search" || item === "group") {
            propertySearchFilter[item] = "";
          }
          // else if (item === "group") {
          // }
          else {
  ...
  getAllParams() {
    ...
    console.log("initialPropertySearchFilter===>", textSearches);
