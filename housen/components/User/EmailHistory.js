	...
	console.log("yes",currentPage);      
           const getSavedSearch = async () => {
				...
				let token = localStorage.getItem('login_token');
                    ...
                    try {
						const json = await requestToAPI(body,"api/v1/services/sentEmailHistory","POST",token);
				 ...
				 {savedData.map((item, key)=>{
				 	...
				 	return(
					 	<tr key={key}>
					      ...
					      <td>{sentDate}</td>
					      <td><img src={question.src} className="hoverAble" onClick={()=>getDetail(item.emailContent)}  width="25px"  title="See detail"/></td>
					    </tr>
				 	);
				 })}
