...
class UserLogin extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            $user = LeadsModel::create($formData);
            // send data to zapier
            $arr = array(
                'Subject' => 'New signup',
                'Name' => $Name,
            ...
            $EmailSubject = $sent_content ? $sent_content->subject : env('APP_NAME');
            $EmailContent = str_replace($content, $content1, $sent_content ? $sent_content->content : env('APP_NAME'));

            $subject = $EmailSubject;

            $EmailContent2 = "<p>Your confirmation code is : " . $code . "</p>";
            // $msg = "<h1>Hi " . $Name . "</h1>
            ...
            $superAdminEmail = getAdmin($agentId);
            sendEmail("SMTP", env('MAIL_FROM'), $request->Email, "", "", $subject, $EmailContent, "UserLogin - Signup", $request->AgentId, env('SIGNUP'));
            ...
            saveNotificationData($notification_data);   ///RAAAAMM
        }
    ...
    public function loginSocial(Request $request)
    {
        ...
        if ($user) {
            ...
            $json['EmailIsVerified'] = $user->EmailIsVerified;
    ...
    public function loginAgent(Request $request, $isRegister = false)
    {
        ...
        if ($validator->passes()) {
            ...
            if ($user) {
                ...
                $json['login_mobile'] = Auth::user()->Phone;
                $json['EmailIsVerified'] = $user->EmailIsVerified;
                $responseArray['estimated_token_time'] = date('Y-m-d H:i:s', strtotime('now +131400 minutes'));
                $strtotime = strtotime($responseArray["estimated_token_time"]);
                $responseArray["estimated_token_time"] = $strtotime;
    ...
    public function duplicateEmail(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // $EmailContent.="<p>Your confirmation code is : ".$code."</p>";
            // $subject = "Confirm Email";
            // $superAdminEmail = getSuperAdmin();
            // sendEmail("SMTP", env('MAIL_FROM'), $request->email, $request->email, $superAdminEmail, $subject, $EmailContent, "UserLogin - Signup", $request->AgentId,env('SIGNUP'));
            return 1;
        }
    }
    public function getWatchProperty(Request $request)
    {
        try {
            $res = [];
            $response_data = SavedSearchFilter::select("ListingId","watchListings","userId")->where('ListingId', $request->ListingId)->where('userId', $request->userId)->get();
            $res['watchList'] = $response_data;
            return response()->json($res, 200);
        } catch (\Throwable $th) {
            return response($th, self::DB_ERROR_HTTP_RESPONSE_STATUS);
        }
    }
    public function forgotPassword22(Request $request)
    {
        $validator = Validator::make($request->all(), [
        ...
        if ($email_exist) {
            ...
            // }
            $forgetLink = "<a style='background-color: #3777a1;
            border: none;
            color: white;
            padding: 15px 32px;
            text-align: center;
            ...
            $content1 = [$Name, $AgentEmail, $AgentPhone, $forgetLink, '', '', '', '', '', '', '', '', env('APP_NAME'), '', env('HOUSENFRONTURL')];

            $EmailContent = str_replace($content, $content1, count($sent_content) > 0 ? $sent_content->content : env('APP_NAME'));
            $EmailSubject = count($sent_content) > 0 ? $sent_content->subject : "Password reset link";
            $subject = $EmailSubject;
            $message = $EmailContent;
            // $subject = 'Password reset link';
            // $message = "Here is your password link to reset your password <a style='background-color: #3777a1;
            // border: none;
            // color: white;
            // padding: 15px 32px;
            // text-align: center;
            ...
            // margin: 4px 2px;
            // cursor: pointer;' href='" . $redirect_url . "'>Click here </a>";
            $sent_token_url = sendEmail("SMTP", env('MAIL_FROM'), $AgentEmail, env('ALERT_CC_EMAIL_ID'), env('ALERT_BCC_EMAIL_ID'), $subject, $message, "UserLogin->forgotPassword", "", env('FORGETPASSWORD'));
            if ($sent_token_url) {
    ...
    public function forgotPassword(Request $request)
    {
        ...
        if ($email_exist) {
            ...
            $sent_content = count($sent_content) > 0 ? $sent_content[0] : "";
            $EmailContent = str_replace($content, $content1, $sent_content->content  ? $sent_content->content : env('APP_NAME'));
            $EmailSubject = $sent_content ? $sent_content->subject : "Password reset link";
    ...
    public function leadForm(Request $req)
    {
        ...
        try {
            ...
            $zap = ZapierSender($arr,$req->agent_id);
            // end code
            //Uncomment from 628 to 643 for making email content dynamic
            // $sent_content = TemplatesModel::select('subject','content')->where('name', 'like', '%' . 'New schedules' . '%' )->first();
            ...
            // $content1 = [$Name,$LeadEmail,$LeadPhone,'','','','','',env('APP_NAME'),env('HOUSENFRONTURL')];
            // $EmailSubject = $sent_content->subject;
            // $EmailContent = str_replace($content,$content1,$sent_content->content);
            // $msg = $EmailContent;
            // $subject = $EmailSubject;

            $superAdminEmail = getAdmin($req->agent_id);
            $subject = "A new enquiry raised";
            sendEmail("SMTP", env('MAIL_FROM'), $superAdminEmail, env('ALERT_CC_EMAIL_ID'), env('ALERT_BCC_EMAIL_ID'), $subject, $msg, "UserLogin - lead form", 2, env('SCHEDULESHOWING'));
            $notification_data = [
    ...
    public function getSavedSearch_bcp24aug(Request $request)
    {
        ...
        try {
            ...
            $offset = $currentPage - 1;
            $start = ($offset * $limit);
            $query = SavedSearchFilter::where('userId', $userId)->where('agentId', $agentId);
            $total = $query->count();
            $lastPage = $total % $limit;
            if ($lastPage == 0) {
                $totalPages = $total / $limit;
        ...
        try {
            ...
            if ($request->isWatch) {
                if ($request->isSold) {
                    $query = SavedSearchFilter::where('userId', $userId)->where('agentId', $agentId)
                        ->where('watchListings', '!=', '')
                        ->with(['propertyListSold' => function ($q) {
                            $q->select(PropertyConstants::SELECT_DATA);
                        }]);
                } else if ($request->isCommunity) {
                    $query = SavedSearchFilter::where('userId', $userId)->where('agentId', $agentId);
                    // ->where('watchListings', '!=', '')
                    // ->with(['propertyListSold' => function ($q) {
                    //     $q->select(PropertyConstants::SELECT_DATA);
                    ...
                    $query = SavedSearchFilter::where('userId', $userId)->where('agentId', $agentId)
                        ->where('watchListings', '!=', '')
                        ->with(['propertyList' => function ($q) {
                            $q->select(PropertyConstants::SELECT_DATA);
                        }]);
                }
            } else {
                $query = SavedSearchFilter::where('userId', $userId)->where('agentId', $agentId)->where('filterName', '!=', "");
            }
            $total = $query->count();
            $lastPage = $total % $limit;
            if ($lastPage == 0) {
            ...
            foreach ($response_data as $resp_data){
                if (collect($resp_data["propertyListSold"])->count() != 0) {
                    foreach ($resp_data["propertyListSold"] as &$dataum) {
                        $dataum = getDom($dataum);
                    }
                }
            }
            foreach ($response_data as $resp_dataa){
                if (collect($resp_dataa["propertyList"])->count() != 0) {
                    foreach ($resp_dataa["propertyList"] as &$dataum) {
                        $dataum = getDom($dataum);
                    }
                }
            }
            if ($request->isCommunity) {
                $days = 24;
                $retPropQuery = RetsPropertyData::query();
                $retPropPurgeQuery = RetsPropertyDataPurged::query();
                foreach ($response_data as $key => $value) {
                    $watchListings = json_decode($value->watchListings, true);
                    if (isset($watchListings['Community']) && $watchListings['Community'] !== '') {
                        // $datas = $retPropQuery->select('City')->where('Community', $watchListings['Community'])->first();
                        $countActive = $retPropQuery->where('Community', $watchListings['Community'])->where('inserted_time', '>=', Carbon::now()->subDays($days))->count();
                        $countSold = $retPropPurgeQuery->where('Community', $watchListings['Community'])->where('inserted_time', '>=', Carbon::now()->subDays($days))->count();
                        $watchListings['countActive'] = $countActive;
                        $watchListings['countSold'] = $countSold;
                        // $watchListings['Community'] = $datas->City . ' - ' . $watchListings['Community'];
                        $value->watchListings = $watchListings;
                        $value->createdAt = Carbon::parse($value->createdAt)->format('Y-m-d');
                    }
                }
            }
            $res['total'] = $total;
    ...
    public function deleteSavedSearch(Request $request)
    {
        ...
        try {
            ...
            if($request->isWatchListing){
                $res = SavedSearchFilter::where("userId", $request->userId)->where("ListingId", $request->ListingId)->delete();
            }
            // $res = true;
    ...
    public function confirmCode(Request $request)
    {
        ...
        if ($data) {
            ...
            $query->where('Email', $email)->update($data);
            return response()->json(["message" => "Email verified successfully", "confirmcode" => true], 200);
        } else {
            return response(["message" => "Incorrect code !", "error" => true], self::VALIDATION_ERROR_HTTP_RESPONSE_STATUS);
        }
    }
    ...
    public function AddUserPreference(Request $request)
    {
        ...
        if ($form_data['action'] == "Add") {
            unset($form_data['AgentId']);
            unset($form_data['housenProject']);
            unset($form_data['userId']);
            unset($form_data['action']);
            $data['Preference'] =  json_encode($form_data);
            $query = LeadsModel::where('id', $userId)->update($data);
            $res['success'] = "Custom setting saved";
            $query =  LeadsModel::select('Preference')->where('id', $userId)->first();
            if ($query) {
        ...
        try {
            ...
            if ($validator->passes()) {
                ...
                $data = array(
                    ...
                    // 'codeCreatedAt' => Carbon::now()->format('Y-m-d H:i:s'),
                    // 'codeUpdatedAt' => Carbon::now()->format('Y-m-d H:i:s'),
                );
                $query->where('Email', $email)->update($data);
                //Code start for dynamic email content send
                $sent_content = TemplatesModel::select('subject', 'content')->where('name', 'like', '%' . 'New signup' . '%')->first();
    ...
    public function checkuser(Request $request)
    {
        ...
        if ($user) {
            if ($user->EmailIsVerified) {
                return response()->json(["message" => "Finded successfully", "confirmcode" => true], 200);
            } else {
                return response(["message" => "Not found !", "error" => true,"confirmcode" => true], self::VALIDATION_ERROR_HTTP_RESPONSE_STATUS);
            }
        } else {
            return response(["message" => "Not found !", "error" => true,"confirmcode" => true], self::VALIDATION_ERROR_HTTP_RESPONSE_STATUS);
        }
