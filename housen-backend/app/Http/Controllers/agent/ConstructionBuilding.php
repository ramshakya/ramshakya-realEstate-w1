...
class ConstructionBuilding extends Controller
{
    ...
    public function get_preconstruction_building(Request $request)
    {
        ...
        $records = $query->get(['id','BuildingName','Address','City','Status','LatestOrComminSoon']);
        $data_array = array();
        ...
        foreach ($records as $key => $record) {
            ...
            // latest and comming soon button
            if($record->LatestOrComminSoon==1){
                $latest="Latest";
            }
            if($record->LatestOrComminSoon==0){
                $latest="Comming Soon";
            }
            if($record->LatestOrComminSoon==2){
                $latest="None";
            }
            ...
            $data_arr['LatestCommingSoon']=$latest;
    ...
    public function add_builder_data(Request $request)
    {
        ...
        else
        {
         ...
         if($request->hasFile('SecondLogo'))
        {
            $logo = $request->file('SecondLogo');
            $ext = $logo->getClientOriginalExtension();
            $filename = time().uniqid().'.'.$ext;
            // $path = 'public/img';
            $logo->move('storage/',$filename);
            // $imagePath =$request->file('Logo')->storeAs($path,$filename);
            $form_data['SecondLogo']=url('storage/'.$filename);
        } 
        else
        {
            unset($form_data['SecondLogo']);
        }
         if($request->hasFile('ThirdLogo'))
        {
            $logo = $request->file('ThirdLogo');
            $ext = $logo->getClientOriginalExtension();
            $filename = time().uniqid().'.'.$ext;
            // $path = 'public/img';
            $logo->move('storage/',$filename);
            // $imagePath =$request->file('Logo')->storeAs($path,$filename);
            $form_data['ThirdLogo']=url('storage/'.$filename);
        } 
        else
        {
            unset($form_data['ThirdLogo']);
        }
    ...
    public function add_construction_data(Request $request)
    {
        ...
        if($request->hasFile('BuildingLogo'))
        {
            $logo = $request->file('BuildingLogo');
            $ext = $logo->getClientOriginalExtension();
            $filename = time().'.'.$ext;
            // $path = 'public/img';
            $logo->move('storage/',$filename);
            // $imagePath =$request->file('Logo')->storeAs($path,$filename);
            $form_data['BuildingLogo']=url('storage/'.$filename);
        } 
        if($request->hasFile('Brochure'))
        {
            $broch = $request->file('Brochure');
            $ext = $broch->getClientOriginalExtension();
            $filename = time().'.'.$ext;
            $broch->move('storage/',$filename);
            $form_data['Brochure']=url('storage/'.$filename);
        } 
        if($request->hasFile('Feature_and_Finishes'))
        {
            $feat = $request->file('Feature_and_Finishes');
            $ext = $feat->getClientOriginalExtension();
            $filename = time().'.'.$ext;
            $feat->move('storage/',$filename);
            $form_data['Feature_and_Finishes']=url('storage/'.$filename);
        } 
