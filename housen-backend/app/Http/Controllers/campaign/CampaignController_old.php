    ...
    public function campaign() {
        ...
        if($data['campaign']){
            foreach ($data['campaign'] as &$k){
                ...
                $k->agents=AssignmentModel::select("ListAgentMlsId","ListAgentFullName","ListAgentEmail")->whereIn('ListAgentMlsId',$agent)->get();
                // return $k->agents;
            }
        }
...
//        $form_data['agent_ids']=json_encode($form_data['agent_ids']);
    ...
    public function getBoard(Request $request){
        $form_data = $request->all();
        $data['board']=AssignmentModel::selectRaw('count(*) as total,id, ListAOR')->where('mls_no',$form_data['msl_no'])->groupBy('ListAOR')->get();
        return $data;
    }
    // Get Agent type
    public function getAgentType(Request $request)
    {
        ...
        if(isset($form_data['boards'])){
            ...
            $data['board']=AssignmentModel::selectRaw('count(*) as total,id, AgentLeadType')->where('mls_no',$form_data['msl_no'])->whereIn('ListAOR', $boardid)->groupBy('AgentLeadType')->get();
        }else{
            $data['board']='';
        }
...
//    Get getleadDataCamp
    ...
    public function getAgentoffice(Request $request)
    {
        $form_data = $request->all();
        if(isset($form_data['agenttype'])){
            $data['board']=AssignmentModel::selectRaw('count(*) as total,ListOfficeMlsId, ListOfficeName,ListAgentMlsId')->where('mls_no',$form_data['msl_no'])->where('AgentLeadType',$form_data['agenttype'])->whereIn('ListAOR', $form_data['boards'])   ->groupBy('ListOfficeMlsId')->orderBy('total','DESC')->get();
        }else{
            $data['board']='';
        }
        return $data;
...
//        }
    ...
    public function getAgent(Request $request)
    {
        ...
        if(isset($form_data['office'])){
            $data['board']=AssignmentModel::selectRaw('count(*) as total,ListAgentMlsId, ListOfficeMlsId,ListAgentFullName')->where('mls_no',$form_data['msl_no'])->where('AgentLeadType',$form_data['agenttype'])->whereIn('ListOfficeMlsId',$form_data['office'])->whereIn('ListAOR', $form_data['boards'])->groupBy('ListAgentMlsId')->get();
        }else{
            $data['board']='';
        }
        return $data;
            ...
            if(!empty($query->all())){
                ...
                if(!empty($agents) && count($agents)>0){
                    foreach($agents as $agent){
                        ...
                        $query_agent=AssignmentModel::where('ListAgentMlsId',$agent_id)->first();
                        // return $query_agent;
                        ...
                        if(collect($query_agent)->count()){
                            ...
                            //$res= CampaignLogsModel::Create($camp_cron_data);
                            // if($template_id==43){
