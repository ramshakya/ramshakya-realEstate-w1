...
use App\Models\SqlModel\DadePropertiesMaster;
use App\Models\SqlModel\AtomAddressMissingSql;

use App\Models\SqlModel\PropAddressFilterSql;
    ...
    public $retsPropertyData;
        public function __construct() {
            ...
            if ($db == "sql"){
                ...
                $this->retsPropertyDataPurged=new PropertyData();
                $this->LeadsModel = new LeadsModel();
                $this->PostalMasterModel = new PostalMasterModel();
...
//        $totalRecords = LeadsModel::select('count(*) as allcount');
        $query=\App\Models\RetsPropertyData::select('count(*) as allcount');
        ...
        if(isset($getdata['search'])){
            ...
            $query=$query->where(function($q) use ($search) {
                $q->where('ListingId', 'like', '%' . $search . '%')
                    ...
                    ->orWhere('PropertyType', 'like', '%' . $search . '%');
            });
        }
...
//        return $totalRecordswithFilter;
        ...
        $type=$getdata['type'];
        $query=\App\Models\RetsPropertyData::where('PropertyType',$type);
        if(isset($getdata['search'])){
            $search=$getdata['search'];
            $query=$query->where(function($q) use ($search) {
                $q->where('ListingId', 'like', '%' . $search . '%')
...
//            $images=explode(',',$query->ImagesUrls);
            foreach ($images as $img){
                ...
                $inner.="'><img class='img-size' src='".$img."' alt='First slide' /></div>";
                $i++;
            }
            $data['inner']=$inner;
          ...
          if( isset($request->token) && !empty($request->token) ){
              ...
              if($token->login_user_id){
                  ...
                  if( isset($apikey) && !empty($apikey) && $apikey==$ApiKeyData){
                      ...
                      //$query=::where('County',$county)->where('CustomPropertyType',$CustomPropertyType);
                      //$query=::where('County',$county)->where('CustomPropertyType',$CustomPropertyType);
//                      return $listingId;
...
//        return $message;
    ...
    public function AddProperty($id=null){
        ...
        $data['Googleapikey']=env('Google_Api_Key');
        foreach ($FeatureType as &$value)
        {
            $value['features']=FeaturesMaster::where('Type',$value['type'])->where('AdminId',0)->get();
        }
