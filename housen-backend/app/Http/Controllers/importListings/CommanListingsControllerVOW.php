...
class CommanListingsControllerVOW extends Controller
{
    //
    public $mls_config;
    ...
    public function importPropertyListing()
    {
        ...
        foreach ($all_mls as $curr_mls_id => $curr_mls) {
            ...
            foreach ($property_class as $className => $classconfig) {
                ...
                if ($diff > 3600 * 24 * 60) {
                    $new_starttime = strtotime("+60 day", $old_timestamp);
                }
                ...
                //$rets_query = "((Timestamp_sql=2022-08-01T01:40:03+))";
                //$rets_query = "((Status=|A),(Timestamp_sql=2020-08-01T01:40:03+))";
                //$rets_query = "((Status=|A))";
                ...
                if ($total_property_count > 0) {
                    ...
                    if ($rets->NumRows($search_chunks) > 0) {
                        ...
                        while ($record = $rets->FetchRow($search_chunks)) {
                            ...
                            if ($record["Status"] == "U"){
                                $table_name =  $property_table_name."Purged";
                                //echo "\n".$table_name;
                            }
...
//                                }
                            ...
                            $rets_data = DB::table($table_name)->where($key_field, $record[$key_field])->get();
                            ...
                            if (count($rets_data) > 0) {
                                ...
                                $rets_data = DB::table($table_name)->where($key_field, $record[$key_field])->update($property_data);
                                $update_prop++;
                                ...
                                $property_data["Reimport"] = 1;
                                DB::table($table_name)->insertGetId($property_data);
