...
class ListingsSoldControllerVOWtest extends Controller
{
    //
    public $mls_config;
            ...
            foreach ($property_class as $className => $classconfig) {
                ...
                if ($total_property_count > 0) {
                    ...
                    if ($rets->NumRows($search_chunks) > 0) {
	    	            ...
	    	            $sold_data = DB::table("RetsPropertyData")->where($key_field_rets, $ml_num)->get();
                            $sold_Data_insert = array();
                            if (count($sold_data) > 0) {
                                foreach ($rets_purged_arr as $key => $value) {
                                    if (isset($sold_data[0])) {
                                        $sold_Data_insert[$key]=$sold_data[0]->$value ? $sold_data[0]->$value:"";
                                    }
                                    $sold_Data_insert['Status'] ="U";
                                }
                               RetsPropertyDataPurged::updateOrCreate(["ListingId" => $ml_num], $sold_Data_insert);
                            }
                            $key_field_resi = "Ml_num";
                            $sold_data_resi = DB::table($property_active_table_name)->where($key_field_resi, $ml_num)->get();
                            $sold_Data_insert = array();
                            if (count($sold_data_resi) > 0) {
                                foreach ($property_data_mapping as $key => $value) {
                                    if (isset($sold_data[0])) {
                                        $sold_Data_insert[$key]=$sold_data_resi[0]->$value ? $sold_data_resi[0]->$value:"";
                                    }
                                    $sold_Data_insert['Status'] ="U";
                                }
                                $rets_data = DB::table($property_table_name)->where($key_field_resi, $ml_num)->get();
                                if (count($rets_data) > 0) {
                                   $rets_data = DB::table($property_table_name)->where($key_field_resi, $ml_num)->update($sold_Data_insert);
                                } else {
                                    DB::table($property_table_name)->insertGetId($sold_Data_insert);
                                }
                                $delete_query = "DELETE FROM `$property_active_table_name` where Ml_num = '" . $ml_num . "'";
