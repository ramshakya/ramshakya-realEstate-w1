...
class GetImagesSize extends Controller
{
    ...
    public function getSize()
     {
        if (!Schema::hasTable('getImagesSizeSold')) 
        {
            Schema::create('getImagesSizeSold', function($table){
                $table->increments('id');
                $table->string('MLS', 40);
                $table->string('Status', 40);
                $table->string('ImagesSize', 250);
                $table->string('tableName', 250);
           });
        }
        ...
        // $all_properties= array("RetsPropertyDataResiPurged","RetsPropertyDataCondoPurged","RetsPropertyDataResi","RetsPropertyDataCondo");
        $all_properties= array("RetsPropertyDataResi","RetsPropertyDataCondo","RetsPropertyDataComm");
                ...
                if (count($result) > 0) {
                    foreach ($result as $key => $val) {
                        ...
                        if (is_readable($img_dir)){
                            if($table_purged=="RetsPropertyDataResiPurged"){
                                $resicount++;
                            }
                            if($table_purged=="RetsPropertyDataCondoPurged"){
                                $condocount++;
                            }
                            if($table_purged=="RetsPropertyDataCommPurged"){
                                $commcount++;
                            }
                            if(count(scandir($img_dir)) > 2){
                                $ImageSize = $this->format_Size($this->getImageSize($img_dir));
                                echo "This MLS Size is \n $ImageSize";
                                DB::table("getImagesSizeSold")->insert(['MLS' => $ml_num, "Status" => $status,"ImagesSize" => $ImageSize,"tableName" => $table_purged]);
                            }
                        }
        ...
        echo "\n Resi Purged Count ".$resicount;
        echo "\n Condo Purged Count ".$condocount;
        echo "\n Comm Purged Count ".$commcount;
