...
class BackupController extends Controller
{
    ...
    public function index() {
       ...
       /*$property_tables = ["RetsPropertyDataCondo"];
        ...
        foreach ($property_tables as $table) {
            if ($table == "RetsPropertyDataResi") {
                ...
                echo "\n table = ".$table;
                $sql_data = RetsPropertyDataCondoPurged::select(["St","Ml_num","St_dir","St_num","St_sfx","Apt_num","Addr"])->whereNull("StandardAddress")->get();
                $total_properties = collect($sql_data)->count();
                echo "\n total_properties_count = ".$total_properties;
                foreach ($sql_data as $data) {
                    $data = collect($data)->all();
        ...
        exec('mysqldump --user='.env("DB_USERNAME").' --password='.env("DB_PASSWORD").' '.env("DB_DATABASE").' RetsPropertyData > /home/mukesh/public_html/db_backup/'.$rpd_path, $errors_rpd);
        exec('mysqldump --user='.env("DB_USERNAME").' --password='.env("DB_PASSWORD").' '.env("DB_DATABASE").' RetsPropertyDataResi > /home/mukesh/public_html/db_backup/'.$rpd_resi_path, $errors_rpd_resi);
        exec('mysqldump --user='.env("DB_USERNAME").' --password='.env("DB_PASSWORD").' '.env("DB_DATABASE").' RetsPropertyDataComm > /home/mukesh/public_html/db_backup/'.$rpd_comm_path, $errors_rpd_comm);
        exec('mysqldump --user='.env("DB_USERNAME").' --password='.env("DB_PASSWORD").' '.env("DB_DATABASE").' RetsPropertyDataCondo > /home/mukesh/public_html/db_backup/'.$rpd_condo_path, $errors_rpd_condo);
        $rpd_error = "";
        $rpd_resi_error = "";
        $rpd_comm_error = "";
        $rpd_condo_error = "";
        ...
        sendEmail("SMTP", env('MAIL_FROM'), env('ALERT_TO_MAIL'), "mukesh.swami8127@gmail.com", "sagar@peregrine-it.com", 'Backup of Active tables Cron FINISHED - ' . $curr_date_end, $txt, "BackupController index", "", env('RETSEMAILS'));
        echo "\n backup successfully";
    }
}
