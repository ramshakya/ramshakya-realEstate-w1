...
class GenerateXMLFile extends Controller
{
    ...
    /*public function generatePropertyXml() {
        ...
        foreach ($data['propertydata'] as $propertydata) {
            $propertyDetailsLink = env('HOUSENFRONTURL')."propertydetails/";
            $xml_txt .= '
                        <url>
    ...
    public function generateSitemapXml() {
        ...
        $xml_txt .= '
                <sitemap>
                  <loc>'.env('HOUSENFRONTURL').'blog-sitemap.xml</loc>
                </sitemap>
                <sitemap>
                  <loc>'.env('HOUSENFRONTURL').'city-sitemap.xml</loc>
                </sitemap>
                <sitemap>
                  <loc>'.env('HOUSENFRONTURL').'property-sitemap.xml</loc>
                </sitemap>
		<sitemap>
		  <loc>'.env('HOUSENFRONTURL').'pages-sitemap.xml</loc>
                </sitemap>
    ';
    ...
    public function generateBlogXml() {
      ...
      xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9
        ...
        foreach ($data['blogdata'] as $propertydata) {
            $propertyDetailsLink = env('HOUSENFRONTURL')."blogs/";
            $xml_txt .= '
                        <url>
    ...
    public function generateCityXml() {
      ...
      xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9
        ...
        foreach ($data['citydata'] as $propertydata) {
            $propertyDetailsLink = env('HOUSENFRONTURL')."city/";
            $xml_txt .= '
                        <url>
    ...
    public function generatePageXml() {
      ...
      xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9
        ...
        foreach ($data['pagesdata'] as $propertydata) {
            $propertyDetailsLink = env('HOUSENFRONTURL');
            $xml_txt .= '
                        <url>
    ...
    public function generatePropertyXml()
    {
        ...
        http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">';
            ...
            $data['propertydata'] = DB::select("select id,SlugUrl from RetsPropertyData where SlugUrl <> ''  limit " . $limit . " offset " . $offset);
            $lastmod = date("Y-m-d h:i:s");
            $datetime = new \DateTime($lastmod);
            $result = $datetime->format('Y-m-d\TH:i:sP');
            foreach ($data['propertydata'] as $propertydata) {
            ...
            $activeXmlTxt .= '<sitemap>
                  <loc>' . env('HOUSENFRONTURL') . 'property-sitemap' . $file_number . '.xml</loc>
                </sitemap>';
            fwrite($file, $xml_txt);
            fclose($file);
        }
    ...
    public function generateSoldListingXml()
    {
        ...
        http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">';
            ...
            $xml_txt .= '</urlset>';
            $soldXmlTxt .= '<sitemap>
                  <loc>' . env('HOUSENFRONTURL') . 'sold-property-sitemap' . $file_number . '.xml</loc>
                </sitemap>';
            fwrite($file, $xml_txt);
            fclose($file);
        }
