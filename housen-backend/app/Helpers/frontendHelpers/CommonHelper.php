...
function get_auto_sugesstion($field, $text, $isSoldSearch = false)
{
    ...
    if ($field == "StandardAddress") {
        array_push($selectData, "ListingId");
    }
    ...
    if ($field == 'StandardAddress') {
        ...
        if(isset($text[0]) && ctype_alpha($text[0])){
            $query->where($field, 'like', $text . '%');
        }else{
            $query->where($field, 'like', '%' . $text . '%');
        }
...
function getSimilar(
    ...
    if ($city && $community) {
        ...
        if ($Status == 'sale' || $Status == 'sold') {
            ...
            //$query->where('LastStatus', 'Sld');
        }
        if ($Status == 'rent' || $Status == 'Lease') {
            $query->where('PropertyStatus', 'Lease');
            //$query->where('LastStatus', 'Lsd');
        }
        if ($bath != null) {
            $query->where('BathroomsFull', $bath);
        }
        if ($bed != null) {
            $query->where('BedroomsTotal', $bed);
        }
        if ($garage != null) {
            $query->orwhere('Gar', $garage);
        }
        if ($ListingId != null) {
    ...
    if (!count($data)) {
        if ($community) {
            ...
            $query = RetsPropertyData::query();
            $query->select(PropertyConstants::SELECT_DATA);
            if (($Status == 'Sale' || $Status == "sold")  && ($property_status == "A" || $property_status == "U")) {
                $query->where('PropertyStatus', 'Sale');
                //$query->where('LastStatus', "Sld");
            }
            if ($Status == 'rent'  && ($property_status == "A" || $property_status == "U")) {
                $query->where('PropertyStatus', 'Lease');
                //$query->where('LastStatus', 'Lsd');
            }
            if ($Style) {
            ...
            if ($propType) {
                $query->where('PropertySubType', $propType);
            }
            if ($bath != null) {
                // $query->where('BathroomsFull','>=', $bath);
            }
            ...
            if ($Lp_dol != null) {
                // A) If subject property is between $40,000 to $550,000 then +/- $100,000
                // B) If subject property is between $550,000 to $1.1M then +/- $150,000
                // C) If subject property is between $1.1M to $1.5M then +/- $200,000
                // D) If subject property is between $1.5M to $2M then similar price up to +/- $250,000
                // E) If subject property is $2M+ then similar price up to + $500,000

                // 949000
                if ($Lp_dol >= 40000 && $Lp_dol <= 550000) {
                    $priceMin = $Lp_dol - 100000;
                    $priceMax = $Lp_dol + 100000;
                }
                if ($Lp_dol >= 550000 && $Lp_dol <= 1100000) {
                    $priceMin = $Lp_dol - 150000;
                    $priceMax = $Lp_dol + 150000;
                }
                if ($Lp_dol >= 1100000 && $Lp_dol <= 1500000) {
                    $priceMin = $Lp_dol - 200000;
                    $priceMax = $Lp_dol + 200000;
                }
                if ($Lp_dol >= 2000000) {
                    $priceMin = $Lp_dol - 250000;
                    $priceMax = $Lp_dol + 500000;
                }
                //$query->where('ListPrice', '>=', (int)$priceMin);
                //$query->where('ListPrice', '<=', (int)$priceMax);

                // $Lp_dol=$Lp_dol+1500.00;
                // $query->orwhere('ListPrice','<=', (int)$Lp_dol);
                // $Lp_dol=$Lp_dol-1500.00;
                // $query->orwhere('ListPrice','>=', (int)$Lp_dol);
            }
            if ($garage != null) {
                $query->where('Gar', $garage);
            }
            if ($ListingId != null) {
                $query->where('ListingId', "!=", $ListingId);
            }
...
function getTemplate($propData)
{
    ...
    $addr = isset($propertyDetails['Addr']) ? $propertyDetails['Addr'] : 0;
    $price = formatDollars(isset($propertyDetails['Lp_dol']) ? $propertyDetails['Lp_dol'] : 0);
    $PropertyStatus = isset($propertyDetails['S_r']) ? $propertyDetails['S_r'] : "";
    $dom = isset($propertyDetails['Dom']) ? $propertyDetails['Dom'] : 0;
    $image = RetsPropertyData::select("ImageUrl")->where("ListingId", $propertyDetails["Ml_num"])->first();
    $ImageUrl = $image->ImageUrl;
    $PropertyType = isset($propertyDetails['PropertyType']) ? $propertyDetails['PropertyType'] : "";
    $bed = isset($propertyDetails['Br']) ? $propertyDetails['Br'] : 0;
    $bath = isset($propertyDetails["Bath_tot"]) ? $propertyDetails["Bath_tot"] : 0;
    $mls = $propertyDetails['Ml_num'];
    $Ad_text = isset($propertyDetails['Ad_text']) ? $propertyDetails['Ad_text'] : "";
                ...
                <tbody>
                    <tr>
                        <td class="m_-2176963290679972721mlTemplateContainer" align="center">
                            <table class="m_-2176963290679972721mlContentTable" width="640" cellspacing="0" cellpadding="0"
                                border="0" align="center">
                                <tbody>
                                    <tr>
                                        <td>
                                        <table class="m_-2176963290679972721mlContentTable" width="640" cellspacing="0"
                                                            ...
                                                            <table class="m_-2176963290679972721mlContentTable"
                                                                ...
                                                                <tbody>
                                                                    <tr>
                                                                        <td>
                                                                            <table role="presentation"
                                                                                style="width:640px;min-width:640px ; margin:2% auto"
                                                                                class="m_-2176963290679972721mlContentTable"
                                                                                width="640" cellspacing="0" cellpadding="0"
                                                                                border="0" align="center">
                                                                                <tbody>
...
function ZapierSender($array_data = null, $agentId = null)
{
    ...
    if($query){
        if($query->WebhookUrl!==''){
            $jsonEncodedData = json_encode($array_data);
...
function getNearest($ListingId, $type = "sold", $community = null, $bed = null,$propType= null,$dom= null,$status=null,$property_status=null,$style=null)
{

    try {
        // $data = getProperties("", $ListingId);
        // if ($data) {
            $table = "";
            $query = "";
            $selectData = [];
            $lastStatus = "";
            $propertyStatus = "";
            if (($property_status=="U" || $property_status=="A") && ($type == "sale" || $type == "sold")) {
                $lastStatus = "Sld";
                $propertyStatus = "Sale";
            } else {
                $lastStatus = "Lsd";
            ...
            /*if ($type === "rent") {
                if (RetsPropertyData::where('ListingId', $ListingId)->exists()) {
                    $table = "RetsPropertyData";
...
//            if ($type === "sold") {
                $query = "";
                $table = "RetsPropertyDataPurged";
                $selectData = PropertyConstants::SELECT_DATA;
                /*if (RetsPropertyDataPurged::where('ListingId', $ListingId)->exists()) {
                    $table = "RetsPropertyDataPurged";
                    $selectData = PropertyConstants::SELECT_DATA;
                }*/
                $query = " where PropertyStatus = '".$propertyStatus."' and LastStatus = '".$lastStatus."'";
           // }
            ...
            if ($community) {
                if ($query) {
                    $query = $query . "  and Community = '$community'";
                } else {
                    $query = " where  Community = '$community'";
                }
            }
            if ($bed) {
                if ($query) {
                    $query = $query . "  and BedroomsTotal = $bed";
                } else {
                    $query = " where  BedroomsTotal = $bed";
                }
            }
            if($propType){
                if ($query) {
                    $query = $query . "  and PropertySubType = '$propType' ";
                } else {
                    $query = " where  PropertySubType = '$propType' ";
                }
            }
            ...
            /*if($dom){
                $c_date = new \DateTime();
                $startDate = $c_date->modify("-$dom day");
                $startDate = $startDate->format('Y-m-d');
                if ($query) {
                    $query = $query . "  and Timestamp_sql < $startDate";
                } else {
                    $query = " where  Timestamp_sql < $startDate";
                }
            }*/
            if ($query) {
                $lastsevenmonth = date("Y-m-d",strtotime("-7 months"));
                //$query = $query . "  and ExpiredDate > '$lastsevenmonth' ";
                $query = $query . "  and Sp_date > '$lastsevenmonth' ";
            } else {
                $lastsevenmonth = date("Y-m-d",strtotime("-7 months"));
                $query = $query . "  where Sp_date > '$lastsevenmonth' ";
                //$query = $query . "  where ExpiredDate > '$lastsevenmonth' ";
            }
            // $Longitude = $data->Longitude;
            // $Latitude = $data->Latitude;
            $limit = PropertyConstants::NEAREST_LIMIT;
            unset($selectData['RoomsDescription']);
            unset($selectData['Ad_text']);
            unset($selectData['Extras']);
            unset($selectData['Cross_st']);
            $selectData = implode(",", $selectData);
            // $qry = "SELECT $selectData, (6371  acos (cos ( radians($Latitude) )  cos( radians( Latitude ) )  cos( radians( Longitude ) - radians($Longitude) ) + sin ( radians($Latitude) )  sin( radians( Latitude ) ) ) ) AS distance   FROM $table  $types  HAVING distance < 1  ORDER BY id desc  LIMIT $limit";
            //$qry = "SELECT $selectData  FROM $table  $query ORDER BY Timestamp_sql desc  LIMIT $limit";
            $qry = "SELECT $selectData  FROM $table  $query ORDER BY Sp_date desc  LIMIT $limit";
            $soldList = DB::select($qry);
            if (collect($soldList)->count() > 0) {
                foreach ($soldList as &$property) {
                    $property = collect($property)->all();
                    $property = getDom($property);
                }
            }
            return $soldList;
        // }
