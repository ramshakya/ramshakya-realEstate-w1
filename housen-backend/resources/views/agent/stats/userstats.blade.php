    ...
    <script type="text/javascript">
        ...
        $(document).on('click','#UserStatsdata',function(){
            ...
            var Filter = $.get('{{url("api/v1/agent/stats/getuserstats_filter")}}',{ 'userid': userid, 'agentid': agentid ,'UserStatsdata':UserStatsdata, 'DateFrom':DateFrom,'DateTo':DateTo} ,function( data ) {
                    ...
                    $('<select id="Users_name" class="form-control" data-placeholder="Choose ..." />').append(option+options).appendTo('#Users');
                    
                    var IpAddress = data.IpAddress;
                    var options = new Array();
                    $.each(IpAddress, function(index, value) {
                        options +=('<option value="' + value['IpAddress'] + '">'+ value['IpAddress'] +'</option>');
                    });
                    $('<select class="form-control" data-placeholder="Choose ..." id="Users_IpAddress" />').append(option+options).appendTo('#IpAddress');
        ...
        $(document).on('change','#datefrom',function(){
            ...
            var Filter = $.get('{{url("api/v1/agent/stats/getuserstats_filter")}}',{ 'userid': userid, 'agentid': agentid ,'UserStatsdata':DateFrom, 'DateFrom':DateFrom,'DateTo':DateTo} ,function( data ) {
                    var users = data.Users;
                    var options = new Array();
                    option = ('<option value=""> -- Select --</option>');
                    $.each(users, function(index, value) {
