...
class MapLoadv2 extends Component {
    ...
    createCluster() {
        ...
        const map = this.map;
        console.log("map==>>2 return");
        map.on("load", () => {
            this.clearPrevMarkers();
            if (map.getLayer('city-new_adv')) {
                map.removeLayer('city-new_adv');
            }
            if (map.getSource('city_adv')) {
                map.removeSource('city_adv');
            }
    ...
    componentDidUpdate(prevProps) {
        ...
        // setTimeout(() => {
            ...
            if (JSON.stringify(this.props.mapData) !== JSON.stringify(prevProps.mapData)) {
                ...
                console.log("map== return from did update");
                return
                this.map.removeLayer("properties_data");
