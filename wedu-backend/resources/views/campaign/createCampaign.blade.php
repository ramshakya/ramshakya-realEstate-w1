                                ...
                                <div class="card-header ">
                                    <div class="row">
                                        ...
                                        <div class="col-4 text-right">
                                                <a href="{{url('agent/campaign/')}}" class="btn btn-purple">All Campaigns</a>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <form class="pro-add-form" method="POST" action="" name="add_emailed" enctype="multipart/form-data" id="campaignForm">
                                            ...
                                            <div class="inline-group">
                                                <div class="row ">
                                                    ...
                                                    <div class="col-md-6">
                                                        <label>Agents Table </label>
                                                        <input type="text" name="" value="BrockerAgents" id="agentTable" class="form-control" readonly>
                                                    </div>
                                                </div>
                                            </div>
                                            <br>
                                            ...
                                            <div class="inline-group">
                                                <div class="row ">
                                                    <div class="col-md-6">
                                                        ...
                                                        <select class="form-control select2" id="mlsData" required>
                                                            <option value="">Select MLS</option>
                                                            @if($mls)
                                                            @foreach($mls as $m)

                                                                <option value="{{@$m['id']}}" <?php if(isset($campaign->mls_no) && $campaign->mls_no==$m['id']){ echo "selected"; }?>>{{@$m['mls']}} </option>
                                                            @endforeach
                                                            @endif
                                                        </select>
                                                    </div>
                                                    <div class="col-md-6">
                                                        ...
                                                        <select class="form-control" multiple="multiple" multiple data-placeholder="Select Boards" id="boardName" required>
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <select class="form-control select2" id="agentType" required>
                                                            ...
                                                            <?php if(isset($type) && !empty($type)){?>
                                                                @foreach($type as $key)
                                                                <option value="{{@$key->AgentLeadType}}" <?php if($campaign->agent_type==$key->AgentLeadType){ echo 'selected';}?>>{{$key->AgentLeadType}}</option>
                                                                @endforeach
                                                            <?php } ?>
                                                        </select>
                                                    </div>
                                                    <div class="col-md-6">
                                                        ...
                                                        <select class="form-control " multiple="multiple" multiple data-placeholder="Select offices" id="agentOffice" required>
                                                            <?php if(isset($office) && !empty($office)){
                                                                ...
                                                                <option value="{{@$key->ListOfficeMlsId}}" <?php if(in_array($key->ListOfficeMlsId,$officeid)){ echo 'selected';}?>>{{$key->ListOfficeName}} - ({{$key->total}})</option>
                                                                @endforeach
                                                            <?php } ?>
                                                        </select>
                                                    </div>
                                            ...
                                            <div class="inline-group">
                                                <div class="row">
                                                    <div class="col-md-11">
                                                        ...
                                                        <select class="form-control select2 select2-multiple" id="agents" multiple="multiple" multiple data-placeholder="Select an agent" required style="height:100px !important;overflow: hidden;">
                                                            <option>Select an agent</option>
                                                            <?php if(isset($agent) && !empty($agent)){
                                                                $agentid=json_decode($campaign->agent_ids);
                                                                ?>
                                                                @foreach($agent as $key)
                                                                <option value="{{@$key->ListAgentMlsId}}" <?php if(in_array($key->ListAgentMlsId,$agentid)){ echo 'selected';}?>>{{$key->ListAgentFullName}}</option>
                                                                @endforeach
                                                            <?php } ?>
                                                        </select>
                                                    </div>
                                                    <div class="col-md-1">
                                                        <span class="btn btn-purple mt-3" id="clearagents"><i class="fa fa-times"></i>
                                                    Clear All</span>
                                                    </div>
                                                </div>
                                            </div>
...
<script src="{{ asset('assets') }}/agent/js/ckeditor/ckeditor.js"></script>
    ...
    <script type="text/javascript">
        ...
        function getOffice(){
            ...
            $.ajax({
                type: "POST",
                url: '{{url("api/v1/agent/campaign/get-agent-office")}}',
                data: data,
                success: function (response) {
                    console.log('response1',response);
                    var html='';
                    $.each(response.board, function(key, val) {
                        html+='<option value="'+val.ListOfficeMlsId+'">'+val.ListOfficeName+' - ('+val.total+')</option>'
                    });
        ...
        function getAgents(){
            ...
            $.ajax({
                type: "POST",
                url: '{{url("api/v1/agent/campaign/get-agent")}}',
                data: data,
                success: function (response) {
                    console.log('response',response);
                    var html='<option value="">Select Agent</option>';
                    $.each(response.board, function(key, val) {
                        html+='<option value="'+val.ListAgentMlsId+'" selected>'+val.ListAgentFullName+' ( Office - '+val.ListOfficeMlsId+' ) </option>'
                    });
                     $('#agents').html(html);
                },
                error:function(status,error){
        ...
        $(document).on('submit','#campaignForm',function(e){
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    setTimeout(function(){
                        window.location.href="{{url('agent/campaign/')}}";
                    },2000);
                },
                error:function(status,error){
