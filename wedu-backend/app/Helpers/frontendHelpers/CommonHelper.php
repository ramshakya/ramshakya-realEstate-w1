...
function get_auto_sugesstion($field, $text, $isSoldSearch = false)
{
    ...
    if (count($data) > 0) {
        ...
        $querySold = \App\Models\PropertyAddressData::query();
        $querySold->select($selectData);
        $querySold->distinct($field);
        $querySold->limit(PropertyConstants::AUTO_SUGGESTION_LIMIT);
        $querySold->where(function ($it) {
            $it->where("Status", "=", "U");
        });
        if ($field == 'StandardAddress') {
            $querySold->where($field, 'like', $text . '%');
        } else {
            $querySold->where($field, 'like', $text . '%');
        }
        $querySold->groupBy($field);
        // DB::enableQueryLog();
        $dataSold = $querySold->get();
        if (count($dataSold) > 0) {
            $result = $dataSold;
        } else {
            $result = [];
...
function getSimilar($community, $type, $priceMin = null, $priceMax = null, $city = null, $PropertySubType = null,$Ml_num =null)
{
    $data = array();
    ...
    $limit = PropertyConstants::SIMILAR_LIST;
    ...
    if(isset($PropertyStatus) && $PropertyStatus!='')
        $query->where('PropertyStatus', $PropertyStatus);
    // if(isset($PropertyStatus) && $PropertyStatus!='')
...
//BY RAM
function weduMapSearchBackUp($condition, $offset, $limit, $sortBy = 'ListPrice', $order = 'ASC', $textSearchField = '', $extra_custom_query = '', $extra_select = '', $shape = '', $array_data = array(), $orFilter = array(), $type = "", $isDefault = false, $isTotal = false)
{
    $isSoldSearch = false;
    $group = "";
    if (isset($condition['group'])) {
    ...
    if ($textSearchField == trim($textSearchField) && str_contains($textSearchField, ' ') &&  $group !== "City") {
        if (strlen($textSearchField) >= 5) {
            $isSoldSearch = true;
            $PurgedTable = RetsPropertyDataPurged::query();
            if ($type == "main") {
    ...
    if (!$total && $group === "ListingId") {
        ...
        if (isset($group)) {
            $PurgedTable->where($group, "$textSearchField");
        }
    }
    if ($isSoldSearch) {
        // $result = $PurgedTable->union($query)->get();
        $result = $PurgedTable->get();
        $total = collect($result)->count();
    } else {
        $result = $query->get();
    }
...
function weduMapSearch($condition, $offset, $limit, $sortBy = 'ListPrice', $order = 'ASC', $textSearchField = '', $extra_custom_query = '', $extra_select = '', $shape = '', $array_data = array(), $orFilter = array(), $type = "", $isDefault = false, $isTotal = false)
{
	...
	$ReqFilters['SqftMin'] = $condition['Sqft'];
        unset($condition['Sqft']);
    }
    if (isset($condition['PropertySubType'])) {
    ...
    foreach ($ReqFilters as $key => $data) {
        ...
        if ($key === "price_min") {
            $query->where('ListPrice', '>=', (int)$data);
            $flags = false;
        }
        if ($key === "price_max") {
            $query->where('ListPrice', '<=', (int)$data);
