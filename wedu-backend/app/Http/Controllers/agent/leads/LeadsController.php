...
use App\Models\SqlModel\PurgeDataModel;
...
use App\Models\SqlModel\BugsReport;
use App\Models\SqlModel\SuggestionlistModel;
...
class LeadsController extends Controller
{
    ...
    public function LeadView($id = null)
    {
        ...
        $LeadEmail = LeadsModel::getLead($id)->first();
        $data['leademail'] = EmailLogs::select("Subject","Content")->where('IsSent',1)->where('ToEmail', $LeadEmail->Email)->orderby("id","desc")->get();        
        $data['propertyView'] = UserTracker::where('UserId', $id)->get();
        $data['Allagent'] =User::select("name","id")->where("type",3)->where("status_id","!=",2)->get();
    ...
    public function ChangeAgent(Request $request)
    {
        ...
        if (isset($request->AssignedAgent) && !empty($request->AssignedAgent)) {
            ...
            $form_data['AssignedAgentName'] = $request->AssignedAgent;
            $agent = [];
            $agent =User::select("name","email","phone_number")->where("id",$request->AssignedAgent)->first();
            if (isset($agent) && !empty($agent)) {
                $form_data['AssignedAgentName'] = $agent->name;
                $form_data['AssgnAgentEmail'] = $agent->email;
                $form_data['AssgnAgentPhone'] = $agent->phone_number;
    ...
    public function AddMail(Request $request)
    {
        ...
        $sendEmail = sendEmail("SMTP", env('MAIL_FROM'), $request->Email, env('ALERT_CC_EMAIL_ID'), env('ALERT_BCC_EMAIL_ID'), $request->Subject, $request->Message, "From leads","",$template_id);
        //        return $unit_id;
    ...
    public function propertyDetails(Request $request)
    {
        ...
        foreach ($propertyView as $record) {
            ...
            $prop= $query->where('SlugUrl', $filtered['slug'])
            ->first();
    ...
    public function PropertiesViewed(Request $request)
    {
        ...
        $propertyView = $query->where('PageUrl', 'like', '%' . env('WEDUURL').'propertydetails/' . '%' )->groupby('PageUrl');
        ...
        foreach ($propertyView as $p) {
            $trim = ltrim($p->PageUrl,env('WEDUURL')."propertydetails/");
            ...
            if (isset($record)) {
                ...
                $data_arr[] = array(
                    ...
                    "Media" => '<img  class=" img-fluid" src="/storage/' . $Imageurl . '" style="height:50px;" onclick="propertyView(' . $qts . $p->PropertyUrl . $qts . ',' . $qts . $p->IpAddress . $qts . ')">',
                ...
                if(isset($purgedproperty)) {
                    ...
                    $data_arr[] = array(
                        'Media' => '<a href="' . $p->PropertyUrl . '" target="_blank"><img  width="100" src="/storage/' . $Imageurl . '"/></a>',
