...
class ImagesControllerSoldVOW extends Controller
{
    ...
    public function imageImport()
    {
        ...
        foreach ($all_mls as $curr_mls_id => $curr_mls) {
            ...
            foreach ($property_class as $className => $classconfig) {
                ...
                $properties_data = DB::table($property_table_name)->where("image_downloaded", "=", 0)->where("image_download_tried", "<", 3)->where('Status', '=', "U")->limit(1000)->get();
                            ...
                            if ($photo["Success"]) {
                                ...
                                if ($img_found) {
                                    ...
                                    if ($photo_number == 1) {
                                        ...
                                        //$rpd_update = RetsPropertyDataPurged::where("ListingId",$mlsId)->update(["ImageUrl" => $fileUrl]);
                                    }
                                } else {
                                    ...
                                    $properties_data = DB::table($property_table_name)->where($key_field, $mlsId)->update(['image_downloaded' => 2, 'image_download_tried' => $image_download_tried, "property_last_updated" => $curr_date, "image_downloaded_time" => $curr_date]);
                                }
   ...
   <tr>
      ...
      <th>Image URL</th>
                ...
                sendEmail("SMTP", env('MAIL_FROM'), $superAdminEmail, env('ALERT_CC_EMAIL_ID'), env('ALERT_BCC_EMAIL_ID'), $subject, $txt, "imagesControllerVow", "3", env('RETSEMAILS'));
            }
        }
    }
    ...
    public function getThumbnail()
    {
        ...
        foreach ($all_mls as $curr_mls_id => $curr_mls) {
            ...
            foreach ($property_class as $className => $classconfig) {
                ...
                //Helper for login
                $rets = mls_login($login_parameter);
                $properties_data = DB::table('RetsPropertyDataPurged')->select(['id', 'ListingId', 'Status', 'Thumbnail_downloaded'])->where("Thumbnail_downloaded", "=", 0)->where('Status', '=', "U")->orderBy('updated_time','DESC')->limit('50000')->get();
