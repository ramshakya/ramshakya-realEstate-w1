...
class ListingsSoldControllerIDXComm extends Controller
{
    //
    ...
    public function importPropertyListing()
    {
        ...
        $file_name = "SoldListingsControllerIDXComm";
            ...
            foreach ($property_class as $className => $classconfig) {
                ...
                if ($total_property_count > 0) {
                    ...
                    if ($rets->NumRows($search_chunks) > 0) {
                        ...
                        if ($temp_listings != []){
                            DB::statement("update RetsPropertyData set needDelete=1 where PropertyType = 'Commercial'");
                            DB::statement("update RetsPropertyDataComm set needDelete=1");
                            foreach ($temp_listings as $temp_listing){
                                echo "\n templistingId = ".$temp_listing["Ml_num"];
                                DB::table($property_table_name)->where("Ml_num",$temp_listing["Ml_num"])->update(["needDelete"=>0]);
                                RetsPropertyData::where("ListingId",$temp_listing["Ml_num"])->update(["needDelete"=>0]);
                                Log::info("This Property is sold now = ".$temp_listing["Ml_num"]." on the date of ".date('Y-m-d'));
                            }
                        }
                        // for main table
                        $active_sold_datas = DB::table($property_table_name)->select("*")->where("needDelete",1)->get();
				    ...
				    echo "\n in update PurgedTable";
                                ...
                                // for images script
                                $images_active = RetsPropertyDataImage::where("listingID",$listingID)->get();
                                if (count($images_active) > 0) {
                                    RetsPropertyDataImagesSold::where("listingID",$listingID)->delete();
                                    foreach ($images_active as $singleImage) {
                                        $singleImage = collect($singleImage)->all();
                                        unset($singleImage["id"]);
                                        RetsPropertyDataImagesSold::create($singleImage);
                                    }
                                }
                            }
                        }
                        // for RPD data
                        $rpd_sold_table_datas = RetsPropertyData::where("needDelete", 1)->get();
                        if (count($rpd_sold_table_datas) > 0){
                            foreach ($rpd_sold_table_datas as $rpd_active_sold_data){
                                ...
                                $rpd_active_sold_data["FromIdxImport"] = 1;
   ...
   <tr>
      ...
      <th>Database Operation</th>
                ...
                sendEmail("SMTP", env('MAIL_FROM'), $superAdminEmail, env('ALERT_CC_EMAIL_ID'), env('ALERT_BCC_EMAIL_ID'), 'Sold IDX Listing Import Cron COMMERCIAL FINISHED - ' . $curr_date_end, $txt, "Listings Sold controller ImportListing", "", env('RETSEMAILS'));
            }
        }
    }
}
