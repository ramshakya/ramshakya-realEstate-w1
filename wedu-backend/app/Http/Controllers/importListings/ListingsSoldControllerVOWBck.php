...
class ListingsSoldControllerVOWBck extends Controller
{
    //
    public $mls_config;
    ...
    public function importPropertyListing()
    {
		...
		$is_run++;
                ...
                $rets_query = "((Status=|U),(Timestamp_sql=2020-10-30T17:55:21-2022-04-30T17:55:21))";
                //$rets_query = "((Status=|U))";
                //$rets_query = "((Status=|U),(Timestamp_sql=2020-04-30T17:55:21+))";
                ...
                //dd($total_property_count,$mls_username);
		if ($total_property_count > 0) {
                    ...
                    echo "\n --" . $offset . " Limit " . $limit . "\n";
                    $search_chunks = $rets->Searchquery($property_resource, $className, $rets_query, array('Format' => 'COMPACT-DECODED', 'Count' => 1, "UsePost" => 1, 'Select' => 'Sp_dol,Ml_num,Cd,Orig_dol'));
                    ...
                    if ($rets->NumRows($search_chunks) > 0) {
                        ...
                        while ($record = $rets->FetchRow($search_chunks)) {
                            ...
                            $property_data["Sp_dol"] = $record["Sp_dol"];
                            $property_data["Cd"] = $record["Cd"];
                            $property_data["Ml_num"] = $record["Ml_num"];

                            $rets_property_data["Sp_dol"] = $record["Sp_dol"];
                            $rets_property_data["Cd"] = $record["Cd"];
                            $rets_property_data["Ml_num"] = $record["Ml_num"];
                            $rets_property_data['mls_no'] = $curr_mls_id;
                            $rets_data = DB::table($property_table_name)->where($key_field, $record[$key_field])->get();
                            echo "\n property detected = " . $record[$key_field];
