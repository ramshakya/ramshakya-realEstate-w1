...
class ListingsSoldControllerTestVOW extends Controller
{
    //
    public $mls_config;
    ...
    {
        $select_data = "Select * from RetsPropertyData where Reimport = 0 and PropertyType = 'Condos' and needDelete <> 1";
        ...
        foreach ($properties_data as $property) {
            ...
            $property["Status"] = "U";
		unset($property["id"]);
            RetsPropertyDataPurged::updateOrCreate(
                ["ListingId" => $property["ListingId"]],
                $property
            );
            $select_data =  RetsPropertyDataCondo::where("Ml_num",$property['ListingId'])->first();
            if ($select_data) {
                $select_data = collect($select_data)->all();
                $select_data["Status"] = "U";
		unset($select_data["id"]);
                RetsPropertyDataCondoPurged::updateOrCreate(
                    ["Ml_num" => $select_data["Ml_num"]],
                    $select_data
                );
                echo "\n big tables also inserted";
            }
            echo "\n Property updated";
            echo "\n Total left propeties = ".$total_property_count--;
        }
	...
	$rets_prop_mapping = [
        ...
        foreach ($all_mls as $curr_mls_id => $curr_mls) {
            ...
            foreach ($property_class as $className => $classconfig) {
                ...
                $total_property_count = 100; //$rets->TotalRecordsFound($search);
   ...
   <tr>
      ...
      <th>Database Operation</th>
        ...
        sendEmail("SMTP", env('MAIL_FROM'), env('SUPERBBROKERTO'), env('ALERT_CC_EMAIL_ID'), env('ALERT_BCC_EMAIL_ID'), 'Sold VOW Listing Import Cron FINISHED - ' . $curr_date_end, $txt, "Sold Listings controller ImportListing", "", env('RETSEMAILS'));
