        ...
        foreach ($all_prop as $key => $value) {
            ...
            foreach ($All_city as $key => $value) {
                if (isset($value->City) && $value->City !="") {
                    ...
                    MarketStats::insert($data);
                    $data = array();
                    $All_community = DB::table("RetsPropertyData")->select("Community")->where("City",$cities)->distinct("Community")->orderBy("Community","ASC")->get();
                    if (isset($All_community) && !empty($All_community) > 0 && isset($All_community[0]->Community) && $All_community[0]->Community !=""){
                        sleep(5);
        ...
        foreach ($all_prop as $key => $value) {
            ...
            foreach ($All_city as $key => $value) {
                if (isset($value->City) && $value->City !="") {
                    ...
                    $othersQuery = "SELECT count(*) as others,DATE_FORMAT(`Sp_date`,'%Y-%m') as date FROM `RetsPropertyDataPurged` where PropertyStatus='Sale' and PropertySubType NOT IN ('Att/Row/Twnhouse','Condo Apt','Detached','Condo Townhouse') and Sp_date<>'0000-00-00' and Sp_date >= '$s_date' $propType $city $community group by date   ORDER BY `date` ASC ";
                    $others = DB::select($othersQuery);
                    foreach ($others as $key => $value) {
                        $month = strtotime("$value->date-01");
                        $months = date("m",$month);
