    ...
    public function mapSearchList(Request $request)
    {
        ...
        if (isset($params['sort_by']) && !empty($params['sort_by'])) {
            ...
            if ($sortData === 'price_high') {
                ...
                $field = 'Timestamp_sql';
                if( $payload["status"] == "Sale" || $payload["status"] == "Lease" && ($payload["propertyType"] == "Residential" || $payload["propertyType"] == "Condos")) {
                        $field = 'ContractDate';
                    }
                elseif(($payload["status"] == "Sold" || $payload["status"] == "Rented") && ($payload["propertyType"] == "Residential" || $payload["propertyType"] == "Condos")){
                    $field = 'Sp_date';
                } else {
                    $field = "Timestamp_sql";
                }
	...
	if (empty($textSearchFilter)) {
        ...
        if ($filteredData['result'] != []) {
            $filteredData['result'] = collect($filteredData['result'])->map(function ($item) {
                ...
                $item = getDom($item);
                if (isset($item["Sp_dol"]) && $item["Sp_dol"] > 0) {
